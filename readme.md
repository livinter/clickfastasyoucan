...
Use JavaScript to create an SVG element with size 500 that displays a twenty-pointed star by alternating between an inner and an outer radius. inner circle with half the radius.
...
Once all circles are clicked:
 ...
 - Show a "save" button that saves the median, label text, and current local date and time to local storage.
